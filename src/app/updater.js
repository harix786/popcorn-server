...
function checkUpdate() {
	...
	if(testInstalled()) {
		...
		var checkVersion = function (ver1, ver2) {
			...
			var num;
			ver1 = _.map(ver1.replace(/[^0-9.]/g, '').split('.'), function (num) {
				num = parseInt(num);
				return Number.isNaN(num) ? 0 : num;
			});
			ver2 = _.map(ver2.replace(/[^0-9.]/g, '').split('.'), function (num) {
				num = parseInt(num);
				return Number.isNaN(num) ? 0 : num;
			});
			...
			for(var i = 0; i < count; i++) {
				if(ver1[i] === undefined) {ver1[i] = 0;}
				if(ver2[i] === undefined) {ver2[i] = 0;}
				if(i === count - 1) {
		...
		request(updateUrl, {json: true}, function (err, res, data) {
			if(err || !data) {
				return;
			} // Its just an updater, we don't care :P
